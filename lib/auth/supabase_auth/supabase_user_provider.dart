...
class ProgressFoundrySupabaseUser extends BaseAuthUser {
  ProgressFoundrySupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> progressFoundrySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ProgressFoundrySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
